...
<div class="max-w-4xl mx-auto space-y-6 animate-fade-in-up">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="relative h-32 bg-gradient-to-r from-[#0F4C81] via-[#1a5f9e] to-[#2563EB]">
            <div class="absolute -bottom-12 left-6">
                @if($user->foto_profil)
                    <img src="{{ asset('storage/' . $user->foto_profil) }}" alt="Foto Profil" class="w-24 h-24 rounded-2xl border-4 border-white shadow-lg object-cover bg-white">
                @else
    ...
    <div x-data="{ tab: '{{ $errors->getBag('updatePassword')->any() ? 'password' : 'info' }}' }">
        <div class="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
            <button @click="tab = 'info'" :class="tab === 'info' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'" class="px-5 py-2.5 rounded-lg text-sm font-medium transition-all duration-200" id="tab-info">
                <span class="flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                    Informasi Profil
                </span>
            </button>
            <button @click="tab = 'password'" :class="tab === 'password' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'" class="px-5 py-2.5 rounded-lg text-sm font-medium transition-all duration-200" id="tab-password">
                <span class="flex items-center gap-2">
        ...
        <div x-show="tab === 'info'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="mt-4">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                <div class="mb-6">
                    <h3 class="text-lg font-bold text-gray-900">Informasi Profil</h3>
                    <p class="text-sm text-gray-500 mt-1">Perbarui data pribadi, foto profil, dan alamat email Anda.</p>
                </div>

                <form method="POST" action="{{ route($profileUpdateRoute) }}" enctype="multipart/form-data" id="form-update-profile">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" value="{{ $roleLabel }}" disabled class="input-field bg-gray-50 text-gray-500 cursor-not-allowed">
                    </div>
                    ...
                    <div class="flex items-center gap-4 pt-4 border-t border-gray-100">
                        ...
                        <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)" class="text-sm font-medium text-emerald-600 flex items-center gap-1">
        ...
        <div x-show="tab === 'password'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="mt-4">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                <div class="mb-6">
                ...
                <form method="POST" action="{{ route($passwordUpdateRoute) }}" id="form-update-password">
                    @csrf
                    @method('PUT')
