    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form action="{{ route('admin.zona.store') }}" method="POST" class="p-6 space-y-5">
            ...
            <div class="space-y-2">
                ...
                <div id="map"></div>
                <input type="hidden" id="geo_boundary" name="geo_boundary" value="{{ old('geo_boundary') }}" required>
                @error('geo_boundary')
                    <p class="mt-1.5 text-xs text-red-600 flex items-center gap-1">
                        <span class="material-symbols-outlined text-sm">error</span>{{ $message }}
                    </p>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Default center (misal Kota Bandung)
            ...
            // Jika ada old('geo_boundary') saat validasi error
            ...
            map.on(L.Draw.Event.CREATED, function (event) {
                ...
                drawnItems.clearLayers(); // Hanya boleh 1 polygon per zona
