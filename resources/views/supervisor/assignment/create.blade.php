                    ...
                    <template x-if="summary.tersedia > 0">
                        <form method="POST" action="{{ route('supervisor.assignment.store', $pengaduan) }}" data-confirm="Yakin ingin menugaskan petugas ini?">
                            ...
                            <div class="mb-5 rounded-xl border border-amber-100 bg-amber-50/40 p-4">
                                ...
                                <textarea name="instruksi" rows="4" class="w-full rounded-xl border border-amber-200/80 bg-white px-3 py-2 text-sm">{{ old('instruksi') }}</textarea>
                            </div>
                            <div class="mb-5">
                                ...
                                <input type="datetime-local" name="jadwal_penanganan"
                                    ...
                                    class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[#022448]">
                            </div>
                            <div class="mb-5">
                                <label class="mb-2 block text-sm font-semibold text-gray-700">Pilih Petugas <span class="text-red-500">*</span></label>
                                <div class="space-y-2">
                                    <template x-for="row in petugas" :key="row.id">
                                        <label class="flex items-center gap-3 rounded-xl border p-3 transition" :class="row.dapat_dipilih ? 'cursor-pointer border-gray-200 hover:bg-gray-50' : 'cursor-not-allowed border-gray-100 bg-gray-50 opacity-70'">
                                            <input type="radio" name="petugas_id" :value="row.id" :disabled="!row.dapat_dipilih">
                                            <div class="flex-1">
                                                <p class="font-semibold text-gray-800" x-text="row.nama"></p>
                                            </div>
                                            <span class="inline-flex shrink-0 items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold" :class="row.status_badge" x-text="row.status_label"></span>
                                        </label>
                                    </template>
                                </div>
                            </div>
                            <div class="flex gap-3">
                                <button type="submit" class="h-11 flex-1 rounded-xl bg-[#022448] font-semibold text-white">Tugaskan Petugas</button>
                            </div>
                        </form>
                    </template>
                </div>
